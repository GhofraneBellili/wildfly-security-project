...
public class TokenEndpoint {
    ...
    public Response token(@FormParam("grant_type")String grantType,
        ...
        if ("refresh_token".equals(grantType)) {
            ...
            var previousRefreshToken = jwtManager.validateJWT(refreshToken);
            if (previousRefreshToken.isPresent()) {
                try {
                    var refreshClaims = previousRefreshToken.get().getJWTClaimsSet();
                    var tenantId = refreshClaims.getStringClaim("tenant_id");
                    var subject = refreshClaims.getSubject();
                    var scopes = refreshClaims.getStringClaim("scope");
                    var roles = refreshClaims.getStringArrayClaim(jwtManager.getClaimRoles());
                    var accessToken = jwtManager.generateAccessToken(tenantId, subject, scopes, roles);
                    var newRefreshToken = jwtManager.generateRefreshToken(tenantId, subject, scopes);

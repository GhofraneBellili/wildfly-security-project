...
public class PushWebSocketEndpoint {
    ...
    @EJB
    private xyz.kaaniche.phoenix.iam.security.JwtManager jwtManager;
    ...
    public void onMessage(JsonObject message,Session session){
        // First message from a client must be an authentication message containing a token
        if (session.getUserProperties().get("authenticated") == null) {
            String token = message.getString("token", null);
            if (token == null) {
                try { session.close(new CloseReason(CloseReason.CloseCodes.VIOLATED_POLICY, "Authentication required")); } catch (Exception ignore) {}
                return;
            }
            var validated = jwtManager.validateJWT(token);
            if (validated.isPresent()) {
                session.getUserProperties().put("authenticated", Boolean.TRUE);
                session.getUserProperties().put("principal", validated.get().getJWTClaimsSet().getSubject());
            } else {
                try { session.close(new CloseReason(CloseReason.CloseCodes.VIOLATED_POLICY, "Invalid token")); } catch (Exception ignore) {}
            }
            return;
        }
        if(session.isOpen() && session.isSecure() && Boolean.TRUE.equals(session.getUserProperties().get("authenticated"))) {

...
public record AuthorizationCode(String tenantName, String identityUsername,
            ...
            try {
                ...
                if (base64Key != null && !base64Key.isEmpty()) {
                    ...
                    if (decoded.length != 16 && decoded.length != 24 && decoded.length != 32) {
                        throw new IllegalArgumentException("authorization.code.key must be 16/24/32 bytes when base64-decoded");
                    }
                    key = new SecretKeySpec(decoded, "AES");
                } else {
                    // fallback to generated AES-256 key (not ideal for multi-instance deployments)
                    byte[] genKey = new byte[32];
                    new SecureRandom().nextBytes(genKey);
                    key = new SecretKeySpec(genKey, "AES");
                }
            } catch (Exception ex) {
                byte[] genKey = new byte[32];
                new SecureRandom().nextBytes(genKey);
    ...
    public String getCode(String codeChallenge) throws Exception {
        ...
        byte[] nonce = AesGcmHelper.getNonce();
        byte[] cipher = AesGcmHelper.encrypt(payloadBytes, key, nonce, associatedData);
    ...
    public static AuthorizationCode decode(String authorizationCode, String codeVerifier) throws Exception {
        ...
        byte[] plain = AesGcmHelper.decrypt(cipher, key, associatedData);

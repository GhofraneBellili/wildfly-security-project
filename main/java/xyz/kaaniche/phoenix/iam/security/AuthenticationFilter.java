    ...
    public void filter(ContainerRequestContext requestContext) {
        ...
        try {
            ...
            if(jwt.isPresent()){
                ...
                // set roles and tenant context for downstream authorization checks
                if (roles != null) {
                    IdentityUtility.setRoles(new java.util.HashSet<>(java.util.Arrays.asList(roles)));
                }
                String tenantId = null;
                try { tenantId = claims.getStringClaim("tenant_id"); } catch(Exception ignored){}
                if (tenantId != null) IdentityUtility.tenantWithName(tenantId);
